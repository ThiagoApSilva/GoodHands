...
<html lang="pt-br">
	...
	<body>
		<?php
			...
			if($informacao['categoria']=="cliente"){
			...
			<section class="pglogin">
				<section style="background-color: rgba(255, 153, 204, 1); padding: 20px; border-radius: 20px;">
					<form name="Login" method="POST" action="#" enctype="multipart/form-data">
						<h4 align="center" class="text-light" style="font-family: Berlin Sans FB">Alterar Cliente</h4>
							...
							<a href="perfil_cliente.php" class="corlink text-light">Voltar</a>
						</div>
					</form>
					<?php
...
//------------------------------------------------------------------------								
							$sqlin = 'UPDATE cadastro set nome="'.$nome.'", usuario="'.$usuario.'", dt_nascimento="'.$dt_nascimento.'", email="'.$email.'", telefone="'.$telefone.'" where id_usuario='.$_SESSION['id_user'].';';

							$inserir= mysqli_query($conexao, $sqlin);	
							if($inserir){
								echo('<script>window.alert("Dados alterado com sucesso!");window.location="perfil_cliente.php";</script>');
							}else{
...
//------------------------------------------------------------------------								
							$sqlin = 'UPDATE cadastro set nome="'.$nome.'", usuario="'.$usuario.'", dt_nascimento="'.$dt_nascimento.'", email="'.$email.'", telefone="'.$telefone.'" where id_usuario='.$_SESSION['id'].';';

							$inserir= mysqli_query($conexao, $sqlin);	
							if($inserir){
								echo('<script>window.alert("Dados alterado com sucesso!");window.location="adm.php";</script>');
							}else{
